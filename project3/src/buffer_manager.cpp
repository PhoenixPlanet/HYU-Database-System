...
pagenum_t BufferManager::get_new_node(uint32_t table_id) {
    ...
    page_t& header = start_use_node(table_id, 0, header_block_idx);
    
    pagenum_t new_node_pagenum = file_manager_.get_new_node(table_id, header);
    
    int new_node_block_idx;
    page_t& new_node = start_use_node(table_id, new_node_pagenum, new_node_block_idx);
...
void BufferManager::delete_node(uint32_t table_id, pagenum_t pagenum) {
    ...
    page_t& target_node = start_use_node(table_id, pagenum, target_block_idx);
...
page_t& BufferManager::start_use_node(uint32_t table_id, pagenum_t pagenum, int& block_idx) {
    int target_idx;
    ...
    buffer_[target_idx].pin_count_++;
    block_idx = target_idx;
    return buffer_[target_idx].frame_;
}
...
int BufferManager::evict_buffer_block() {
    int target_idx;
    if (buffer_[buffer_tail_].pin_count_ != 0) {
        bool find_target = false;
        while (!find_target) {
            for (int i = buffer_tail_; i != HEAD_IDX; i = buffer_[i].prev_block_) {
                if (buffer_[i].pin_count_ == 0) {
                    target_idx = i;
                    find_target = true;
                    break;
                }
...
void BufferManager::erase_buffer_block(int target_idx) {
    while (buffer_[target_idx].pin_count_ > 0);
