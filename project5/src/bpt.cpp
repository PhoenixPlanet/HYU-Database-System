...
int BPT::find(uint32_t table_id, int64_t key, record_t& record, int trx_id) {
    ...
    for (int i = 0; i < number_of_keys; i++) {
        if (leaf.read_data()[i].key_ == key) {
            ...
            TransactionManager::instance().addLog(trx_id, TrxLog(LogType::READ, table_id, target_pagenum, key));
            return 0;
        }
    }
...
int BPT::update(uint32_t table_id, int64_t key, char* values, int trx_id) {
    ...
    for (int i = 0; i < number_of_keys; i++) {
        if (leaf.read_data()[i].key_ == key) {
            ...
            leaf_again.end_use_node();
            TransactionManager::instance().addLog(trx_id, TrxLog(LogType::UPDATE, table_id, target_pagenum, key, original, std::string(values)));
            return 0;
        }
    }
