...
struct TrxLog {
    ...
    TrxLog(LogType log_type, uint32_t table_id, pagenum_t pagenum, int64_t key, std::string before, std::string after, int64_t prev_lsn, int64_t lsn)
        : log_type_(log_type),
          table_(table_id),
          page_(pagenum),
          key_(key),
...
public:
    ...
    int64_t getPrevLSN(int trx_id);
    void setPrevLSN(int trx_id, int64_t prev_lsn);
