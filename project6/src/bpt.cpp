...
// 찾았을 때 true 아닐 때 false return
int BPT::find(uint32_t table_id, int64_t key, record_t& record, int trx_id) {
    if (!TransactionManager::instance().isTrxExistWithLock(trx_id)) {
        return 1;
    }
    ...
    for (int i = 0; i < number_of_keys; i++) {
        if (leaf.read_data()[i].key_ == key) {
            pagenum_t target_pagenum = leaf.get_pagenum();
            leaf.end_use_node();

            TrxFlag lock = LockManager::instance().lock_acquire(table_id, key, trx_id, LockMode::SHARED);
            if (lock != TrxFlag::SUCCESS) {
                return 3;
            }
            
            BPTNode<NodeType::LEAF> leaf_again(table_id, target_pagenum);
            record = leaf_again.read_data()[i];
            leaf_again.end_use_node();
            return 0;
        }
    }
    
    leaf.end_use_node();
    TransactionManager::instance().abortTrx(trx_id);
    return 4;
}

int BPT::update(uint32_t table_id, int64_t key, char* values, int trx_id) {
    if (!is_db_initialized_) {
        #ifdef DEBUG_BPT
        perror("DB not initialized");
        #endif
    ...
    for (int i = 0; i < number_of_keys; i++) {
        if (leaf.read_data()[i].key_ == key) {
            ...
            TrxFlag lock = LockManager::instance().lock_acquire(table_id, key, trx_id, LockMode::EXCLUSIVE);
            if (lock != TrxFlag::SUCCESS) {
                return 3;
            }
            
            BPTNode<NodeType::LEAF> leaf_again(table_id, target_pagenum);
            std::string original(leaf_again.read_data()[i].value_);
            strcpy(leaf_again.write_data()[i].value_, values);

            // 로그 발급
            CommonLog log;
            log.prev_lsn_ = TransactionManager::instance().getPrevLSN(trx_id);
            log.trx_id_ = trx_id;
            log.type_ = LogType::UPDATE;
            
            UpdateData data;
            data.table_id_ = table_id;
            data.pagenum_ = target_pagenum;
            data.offset_ = sizeof(page_t::page::node_page::header) + (sizeof(child_t) * i);
            data.data_length_ = 120;
            strcpy(data.old_image_, original.c_str());
            strcpy(data.new_image_, values);
            
            // lsn 페이지에 반영
            int64_t target_lsn = LogManager::instance().addUpdateLog(log, data);
            leaf_again.lsn(target_lsn);

            leaf_again.end_use_node();

            TransactionManager::instance().setPrevLSN(trx_id, target_lsn);
            TransactionManager::instance().addLog(trx_id, TrxLog(LogType::UPDATE, table_id, target_pagenum, key, original, std::string(values), log.prev_lsn_, target_lsn));
            return 0;
        }
    }
...
int BPT::find(uint32_t table_id, int64_t key, char* ret_val, int trx_id) {
    if (!is_db_initialized_) {
        #ifdef DEBUG_BPT
        perror("DB not initialized");
        #endif
    ...
    int flag = find(table_id, key, rec, trx_id);
    if (flag == 0) {
